...
async def create_trip_route(
    ...
    trip = await create_trip(db, user_id=user_id, **data.model_dump())
...
async def list_trips_route(
    ...
    """List trips belonging to the current user."""
    trips = await get_trips(db, user_id=user_id)
    return trips
...
async def get_trip_route(
    ...
    """Get a trip — only accessible by its owner."""
    trip = await get_trip_detail(db, trip_id)
    if not trip:
        raise HTTPException(status_code=404, detail="Trip not found")
    if trip.user_id and trip.user_id != user_id:
...
async def update_trip_route(
    ...
    """Update trip details — only accessible by its owner."""
    existing = await get_trip_detail(db, trip_id)
    if not existing:
        raise HTTPException(status_code=404, detail="Trip not found")
...
async def delete_trip_route(
    ...
    """Delete a trip — only accessible by its owner."""
    existing = await get_trip_detail(db, trip_id)
    if not existing:
        raise HTTPException(status_code=404, detail="Trip not found")
    if existing.user_id and existing.user_id != user_id:
